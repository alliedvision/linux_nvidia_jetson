 ...
 *
 * $Id: dhd_common.c 674206 2017-10-12 07:09:16Z $
 */
#include <typedefs.h>
#include <osl.h>
...
#define WLC_EVENT_MSG_BAND_5G       0x20    /* i/f band is 5G */
#define WLC_EVENT_MSG_WLC1      0x40    /* wlc core of host i/f */
	...
	wl_event_msg_t *event, void **data_ptr, void *raw_event)
{
	...
	case WLC_E_FIFO_CREDIT_MAP:
		...
		dhd_wlfc_FIFOcreditmap_event(dhd_pub, event_data, datalen);
	...
	case WLC_E_IF:
...
#ifdef PROP_TXSTATUS
		{
			...
			WLFC_DBGMESG(("WLC_E_IF: idx:%d, action:%s, iftype:%s, "
			              ...
			              ifevent->ifidx,
			              ((ifevent->opcode == WLC_E_IF_ADD) ? "ADD":
			               ((ifevent->opcode == WLC_E_IF_CHANGE) ? "CHANGE":"DEL")),
...
#ifdef PROP_TXSTATUS
		WLFC_DBGMESG(("WLC_E_LINK: ifidx:%d bsscfgidx:%d wlc-core:%d band:%s LINK %s\n",
				event->ifidx, event->bsscfgidx,
				(flags & WLC_EVENT_MSG_WLC1) ? 1 : 0,
				(flags & WLC_EVENT_MSG_BAND_5G) ? "5GHz" : "2.4GHz",
				(flags & WLC_EVENT_MSG_LINK) ? "UP" : "DOWN"));
		/* Update intf credit group based on band only for now.
		 * TODO: going forward we should have credit updates
		 * based on mimo<->rsdb switch using core info.
		 */
		if (flags & WLC_EVENT_MSG_BAND_5G) {
			dhd_wlfc_update_interface_credit_group(dhd_pub, event->ifidx,
