...
static int wifi_get_mac_addr_file(unsigned char *buf)
{
	...
	if (rdlen > 0)
		fp->f_pos += rdlen;
	if (rdlen != 17) {
		DHD_INFO(("%s: bad mac address file - len %d != 17",
			 __func__, rdlen));
		ret = -ENOENT;
	} else if (sscanf(str, "%x:%x:%x:%x:%x:%x",
		&mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]) != 6) {
		DHD_INFO(("%s: bad mac address file - must contain xx:xx:xx:xx:xx:xx\n",
			__func__));
		ret = -ENOENT;
	} else {
		DHD_INFO(("%s: using wifi mac %02x:%02x:%02x:%02x:%02x:%02x\n",
			 __func__,
	...
	if (!is_valid_ether_addr(buf)) {
		DHD_INFO(("%s: invalid mac %02x:%02x:%02x:%02x:%02x:%02x\n",
			__func__,
...
err_out:
	DHD_INFO(("%s: bad mac address at %s/%s: %s.\n",
