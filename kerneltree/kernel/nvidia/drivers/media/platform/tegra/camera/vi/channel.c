...

{
	...
	if (chan->capture_state != CAPTURE_GOOD) {
		...
		chan->stream_stats.FramesIncomplete++;
		return;
	} else {
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
		/* update time stamp of the buffer */
			...
			bool use_prio, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_STREAMSTAT: {
		...
		chan->stream_stats.FramesUnderrun = chan->qbuf_count - chan->dqbuf_count;
	...
	case VIDIOC_STREAMON_EX: {
		...
		/* Find subdev for CSI */
		for (num_sd = 0; num_sd < chan->num_subdevs; num_sd++)
			if (chan->subdev[num_sd] != chan->subdev_on_csi) {
				csi_chan = to_csi_chan(chan->subdev[num_sd]);
				csi_sd = num_sd;
			}

		if (streamon->nLaneCount > 0 && csi_chan != NULL) {

			struct v4l2_subdev *sd = chan->subdev_on_csi;
			struct camera_common_data *s_data =
				to_camera_common_data(sd->dev);

			/* Set new lane count for all devices */
			s_data->numlanes =
				chan->numlanes =
				csi_chan->numlanes =
				streamon->nLaneCount;

			for (i = 0; i < csi_chan->numports; i++) {

				unsigned int port;

				/* replicate lane count to CSI ports */
				csi_chan->ports[i].lanes = streamon->nLaneCount;

				port = csi_chan->ports[i].csi_port;

				if (csi_chan->numlanes <= 2)
					lanes |= CSIA << port;
				else
					lanes |= (CSIA | CSIB) << port;
			}

			nvcsi_cil_sw_reset(lanes, 1);
			tegra_csi_mipi_calibrate(csi_chan->csi, true);
			nvcsi_cil_sw_reset(lanes, 0);
		}
		...
		ret = vb2_core_streamon(&chan->queue, streamon->nBufferType);
		if (ret < 0)
			return ret;
	...
	case VIDIOC_STREAMOFF_EX: {
		...
		ret = vb2_core_streamoff(&chan->queue, streamoff->nBufferType);
