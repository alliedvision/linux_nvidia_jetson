...
#define DRV_NAME "tegra186-snd-boden"
...
struct tegra_t186ref_boden_amx_adx_conf {
...
struct tegra_t186ref_boden {
	struct tegra_asoc_audio_clock_info audio_clock;
	struct tegra_t186ref_boden_amx_adx_conf amx_adx_conf;
...
static const struct snd_soc_pcm_stream tegra_t186ref_boden_amx_input_params[][4] = {
	{
...
static const struct snd_soc_pcm_stream tegra_t186ref_boden_amx_output_params[] = {
	PARAMS(SNDRV_PCM_FMTBIT_S32_LE, 16),
	PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 4),
	PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 4),
	PARAMS(SNDRV_PCM_FMTBIT_S32_LE, 16),
};
static const struct snd_soc_pcm_stream tegra_t186ref_boden_adx_output_params[][4] = {
	{
		PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 3),
		PARAMS(SNDRV_PCM_FMTBIT_S32_LE, 16),
		PARAMS(SNDRV_PCM_FMTBIT_S32_LE, 16),
...
static const struct snd_soc_pcm_stream tegra_t186ref_boden_adx_input_params[] = {
	PARAMS(SNDRV_PCM_FMTBIT_S32_LE, 8),
	PARAMS(SNDRV_PCM_FMTBIT_S32_LE, 16),
...
static const struct snd_soc_pcm_stream
		tegra_t186ref_boden_sse_admaif_params[] = {
	PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 4),
	PARAMS(SNDRV_PCM_FMTBIT_S16_LE, 2),
};
static int tegra_t186ref_boden_amx_dai_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_card *card = rtd->card;
	struct tegra_t186ref_boden *machine = snd_soc_card_get_drvdata(card);
...
static int tegra_t186ref_boden_adx_dai_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_card *card = rtd->card;
	struct tegra_t186ref_boden *machine = snd_soc_card_get_drvdata(card);
...
static int tegra_t186ref_boden_i2s_dai_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int tegra_t186ref_boden_audio_dsp_tdm1_hw_params(
					...
					struct snd_pcm_hw_params *params)
{
	...
	rtd = snd_soc_get_pcm_runtime(card, "boden-audio-dsp-tdm1-1");
	if (!rtd)
		return -EINVAL;
...
static int tegra_t186ref_boden_audio_dsp_tdm2_hw_params(
					struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	rtd = snd_soc_get_pcm_runtime(card, "boden-audio-dsp-tdm1-2");
	if (!rtd)
		return -EINVAL;
...
static int tegra_t186ref_boden_spdif_hw_params(
					struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
...
static struct snd_soc_ops tegra_t186ref_boden_audio_dsp_tdm1_ops = {
	.hw_params = tegra_t186ref_boden_audio_dsp_tdm1_hw_params,
};

static struct snd_soc_ops tegra_t186ref_boden_audio_dsp_tdm2_ops = {
	.hw_params = tegra_t186ref_boden_audio_dsp_tdm2_hw_params,};

static struct snd_soc_ops tegra_t186ref_boden_spdif_ops = {
	.hw_params = tegra_t186ref_boden_spdif_hw_params,
};

static const struct snd_soc_dapm_widget tegra_boden_dapm_widgets[] = {
...
static int tegra_t186ref_boden_suspend_pre(struct snd_soc_card *card)
{
...
static struct snd_soc_card snd_soc_tegra_t186ref_boden = {
	.name = "tegra-t186ref_boden",
	.owner = THIS_MODULE,
	.suspend_pre = tegra_t186ref_boden_suspend_pre,
	.dapm_widgets = tegra_boden_dapm_widgets,
	.num_dapm_widgets = ARRAY_SIZE(tegra_boden_dapm_widgets),
	.controls = tegra_t186ref_controls,
	.num_controls = ARRAY_SIZE(tegra_t186ref_controls),
	.fully_routed = true,
};

static int tegra_t186ref_boden_driver_probe(struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;
	struct device_node *root;
	struct snd_soc_card *card = &snd_soc_tegra_t186ref_boden;
	struct tegra_t186ref_boden *machine;
	struct snd_soc_dai_link *tegra_machine_dai_links = NULL;
	struct snd_soc_dai_link *tegra_t186ref_boden_codec_links = NULL;
	struct snd_soc_codec_conf *tegra_machine_codec_conf = NULL;
	struct snd_soc_codec_conf *tegra_t186ref_boden_codec_conf = NULL;
	int ret = 0, i, j;
	const char *model;

	machine = devm_kzalloc(&pdev->dev, sizeof(struct tegra_t186ref_boden),
					GFP_KERNEL);
	if (!machine) {
		dev_err(&pdev->dev, "Can't allocate tegra_t186ref_boden struct\n");
		ret = -ENOMEM;
		goto err;
	}
	...
	if (np) {
		...
		if (strcmp(model, "boden_t186")) {
			amx_input_params = tegra_t186ref_boden_amx_input_params;
			amx_output_params = tegra_t186ref_boden_amx_output_params;
			adx_input_params = tegra_t186ref_boden_adx_input_params;
			adx_output_params = tegra_t186ref_boden_adx_output_params;
		} else {
			dev_err(&pdev->dev,
	...
	if (!tegra_t186ref_boden_codec_links)
		goto err_alloc_dai_link;


	tegra_t186ref_boden_codec_conf = tegra_machine_new_codec_conf(pdev,
			tegra_t186ref_boden_codec_conf,
			&machine->num_codec_links);
	if (!tegra_t186ref_boden_codec_conf)
		goto err_alloc_dai_link;
	...
	for (i = 0; i < machine->amx_adx_conf.num_amx; i++)
		tegra_machine_set_dai_init(TEGRA186_DAI_LINK_AMX1 + (5*i),
			&tegra_t186ref_boden_amx_dai_init);
	for (i = 0; i < machine->amx_adx_conf.num_adx; i++)
		tegra_machine_set_dai_init
			(TEGRA186_DAI_LINK_ADX1 + (5*i),
			&tegra_t186ref_boden_adx_dai_init);

	/* set codec init */
	for (i = 0; i < machine->num_codec_links; i++)
		tegra_t186ref_boden_codec_links[i].init =
				tegra_t186ref_boden_i2s_dai_init;
	...
	if (machine->amx_adx_conf.num_amx) {
		for (i = 0; i < machine->amx_adx_conf.num_amx; i++) {
			...
			tegra_machine_set_dai_params
				...
				(struct snd_soc_pcm_stream *)
				&tegra_t186ref_boden_amx_output_params[i]);
		}
	}
	...
	if (machine->amx_adx_conf.num_adx) {
		for (i = 0; i < machine->amx_adx_conf.num_adx; i++) {
			tegra_machine_set_dai_params
				...
				(struct snd_soc_pcm_stream *)
				&tegra_t186ref_boden_adx_output_params[i][0]);
			tegra_machine_set_dai_params
				(TEGRA186_DAI_LINK_ADX1_2 + (5*i),
				(struct snd_soc_pcm_stream *)
				&tegra_t186ref_boden_adx_output_params[i][1]);
			tegra_machine_set_dai_params
				(TEGRA186_DAI_LINK_ADX1_3 + (5*i),
				(struct snd_soc_pcm_stream *)
				&tegra_t186ref_boden_adx_output_params[i][2]);
			tegra_machine_set_dai_params
				(TEGRA186_DAI_LINK_ADX1_4 + (5*i),
				(struct snd_soc_pcm_stream *)
				&tegra_t186ref_boden_adx_output_params[i][3]);
			tegra_machine_set_dai_params
				(TEGRA186_DAI_LINK_ADX1 + (5*i),
				(struct snd_soc_pcm_stream *)
				&tegra_t186ref_boden_adx_input_params[i]);
		}
	}
	...
	for (i = 0; i < 2; i++) {
		tegra_machine_set_dai_params(
			...
			(struct snd_soc_pcm_stream *)
			&tegra_t186ref_boden_sse_admaif_params[i]);
	...
	for (i = TEGRA186_DAI_LINK_ADMAIF1;
		...
		tegra_machine_set_dai_ops(i, &tegra_t186ref_boden_spdif_ops);

	for (i = 0; i < machine->num_codec_links; i++) {
		if (tegra_t186ref_boden_codec_links[i].name) {
			if (strstr(tegra_t186ref_boden_codec_links[i].name,
				"boden-audio-dsp-tdm1-1")) {
				for (j = TEGRA186_DAI_LINK_ADMAIF1;
					j <= TEGRA186_DAI_LINK_ADMAIF4; j++)
					tegra_machine_set_dai_ops(j,
					&tegra_t186ref_boden_audio_dsp_tdm1_ops);
			} else if (strstr
				(tegra_t186ref_boden_codec_links[i].name,
					"boden-audio-dsp-tdm1-2")) {
				for (j = TEGRA186_DAI_LINK_ADMAIF5;
					j <= TEGRA186_DAI_LINK_ADMAIF8; j++)
					tegra_machine_set_dai_ops(j,
					&tegra_t186ref_boden_audio_dsp_tdm2_ops);
			}
		}
	}

	/* append t186ref_boden specific dai_links */
	card->num_links =
...
static int tegra_t186ref_boden_driver_remove(struct platform_device *pdev)
{
...
static const struct of_device_id tegra_t186ref_boden_of_match[] = {
	{ .compatible = "nvidia,tegra-audio-t186ref-boden", },
	{},
};

static struct platform_driver tegra_t186ref_boden_driver = {
	.driver = {
		.name = DRV_NAME,
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.probe_type = PROBE_PREFER_ASYNCHRONOUS,
		.of_match_table = tegra_t186ref_boden_of_match,
	},
	.probe = tegra_t186ref_boden_driver_probe,
	.remove = tegra_t186ref_boden_driver_remove,
};
module_platform_driver(tegra_t186ref_boden_driver);

MODULE_AUTHOR("Dipesh Gandhi <dipeshg@nvidia.com>");
MODULE_DESCRIPTION("Tegra+t186ref_boden machine ASoC driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:" DRV_NAME);
MODULE_DEVICE_TABLE(of, tegra_t186ref_boden_of_match);
