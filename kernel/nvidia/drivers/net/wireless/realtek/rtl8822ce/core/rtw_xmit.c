...
#endif /* CONFIG_BR_EXT */
#ifdef CONFIG_LAYER2_ROAMING
	if ((pmlmepriv->roam_buf_pkt) && (skb->protocol != htons(0x888e))) {	/* eapol never enqueue.*/
		pxmitframe->pkt = *ppkt;
		rtw_list_delete(&pxmitframe->list);
...
void rtw_tx_desc_backup(_adapter *padapter, u8 *txbd, struct xmit_frame *pxmitframe,
	u8 desc_size, u8 hwq, u32 txdma_status)
{
	u32 tmp32;
	u8 *pxmit_buf;
	...
	_rtw_memcpy(tx_backup[hwq][backup_idx[hwq]].tx_bak_txbd, txbd, TX_BAK_TXBD_LEN);
	...
	tx_backup[hwq][backup_idx[hwq]].tx_dma_status = txdma_status;
