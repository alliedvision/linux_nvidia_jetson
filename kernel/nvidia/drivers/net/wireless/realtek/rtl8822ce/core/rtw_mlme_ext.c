...
unsigned int OnProbeRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (pmlmepriv->roam_network)
		RTW_INFO("%s() "MAC_FMT"\n", __func__, MAC_ARG(get_addr2_ptr(precv_frame)));
...
void parse_qbss(_adapter *padapter, u8* ies, int ie_len,  WLAN_BSSID_EX *bssid)
{
	int len;
	u8 *p;
	u8 ie_offset = _FIXED_IE_LENGTH_;

	/* QBSS */
	p = rtw_get_ie(ies + ie_offset, EID_QBSSLoad, &len, ie_len - ie_offset);
	if (p) {
		memcpy(&bssid->qbss_sta_cnt, (void *)p + 2, 2);
		bssid->qbss_ch_util = *(p + 4);
		bssid->qbss_valid = 1;
		#if 0
		RTW_INFO("QBSS "MAC_FMT" util=%d, cnt=%d %s\n" , MAC_ARG(bssid->MacAddress),
			bssid->qbss_ch_util, bssid->qbss_sta_cnt, bssid->Ssid.Ssid);
		#endif
	} else {
		bssid->qbss_sta_cnt = 0;
		bssid->qbss_ch_util = 0;
		bssid->qbss_valid = 0;
	}
	...
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network), ETH_ALEN)) {
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				pmlmepriv->cur_network_scanned->last_scanned = rtw_get_current_time();
				pmlmeext->bcn_cnt++;

				parse_qbss(padapter, pframe + sizeof(struct rtw_ieee80211_hdr_3addr),
					precv_frame->u.hdr.len - sizeof(struct rtw_ieee80211_hdr_3addr),
...
static void recv_ack(_adapter *padapter, bool en)
{
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	u16 reg;

	pmlmepriv->roam_ack = 0;
	reg = rtw_read16(padapter, 0x6a2);
	...
	rtw_write16(padapter, 0x6a2, reg);
...
static unsigned int rtw_roam_nb_discover(_adapter *padapter, u8 bfroce, u32 delay_start)
{
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);	
	...
	if (bfroce || (!pmlmepriv->nb_info.nb_rpt_valid))
		nb_req_issue = _TRUE;
	
	if (nb_req_issue && (psta->rm_en_cap[0] & RTW_RRM_NB_RPT_EN))
		return rm_add_nb_req(padapter, psta, delay_start);
...
#ifdef CONFIG_RTW_80211K
		rtw_roam_nb_discover(padapter, _TRUE, 0);
#endif
	}
#endif
	return _SUCCESS;
...
{
	/* In this case, use 30 ms as the default wait_ack timeout */
	return dump_mgntframe_and_wait_ack_timeout(padapter, pmgntframe, 30);
}
...
#if 0 /* DEBUG */
	if (da && pssid)
		RTW_INFO(FUNC_ADPT_FMT" to %s ("MAC_FMT") ch:%u\n",
			FUNC_ADPT_ARG(padapter), pssid->Ssid, MAC_ARG(da), rtw_get_oper_ch(padapter));
	else if (pssid)
		RTW_INFO(FUNC_ADPT_FMT" to %s (ff:ff:ff:ff:ff:ff) ch:%u\n",
			FUNC_ADPT_ARG(padapter), pssid->Ssid, rtw_get_oper_ch(padapter));
	else
		RTW_INFO(FUNC_ADPT_FMT" to * (ff:ff:ff:ff:ff:ff) ch:%u\n",
...
static u8 * rtw_set_rtk_vendor_ie(_adapter *padapter , u8 *pframe ,uint *frlen ) {
	int vender_len;
	systime current_time;
	unsigned char vendor_info[255];
	unsigned char REALTEK_OUI[] = {0x00, 0xe0, 0x4c};
	unsigned char *ptr;

	current_time = rtw_get_current_time();
	_rtw_memset(vendor_info,0,255);
	_rtw_memcpy(vendor_info, REALTEK_OUI, 3);
	ptr = vendor_info + 3;
	snprintf(ptr , sizeof(vendor_info) - 3, "%s-%ld", DRIVERVERSION, current_time);
	vender_len = strlen(ptr) + 3;

	RTW_INFO("Realtek vendor IE %s\n", ptr);
...
void _issue_assocreq(_adapter *padapter, u8 is_reassoc)
{
	...
	*(fctrl) = 0;
	...
	if (is_reassoc == _TRUE) {
	#ifdef CONFIG_LAYER2_ROAMING
		if (pmlmepriv->roam_network && !is_zero_mac_addr(pmlmepriv->roam_from))
			_rtw_memcpy(pframe, pmlmepriv->roam_from, ETH_ALEN);
		else
	#endif
	...
	pframe = rtw_set_rtk_vendor_ie(padapter, pframe, &pattrib->pktlen);
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	/* QBSS */
	parse_qbss(padapter, bssid->IEs, bssid->IELength, bssid);
	...
	if (caps & cap_ESS) {
		...
		if (rtw_ft_roam(padapter))
			_set_timer(&padapter->mlmepriv.assoc_timer,
				(REAUTH_FT_TO * REAUTH_LIMIT) +
				(REASSOC_FT_TO * REASSOC_LIMIT) + beacon_timeout);
		else
#endif
			_set_timer(&padapter->mlmepriv.assoc_timer,
...
#ifdef CONFIG_RTW_80211R
	if (rtw_ft_roam(padapter))
		set_link_timer(pmlmeext, REASSOC_FT_TO);
	else
#endif
...
unsigned int receive_disconnect(_adapter *padapter, unsigned char *MacAddr, unsigned short reason, u8 locally_generated)
{
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		...
		rtw_clr_dfs_beacon_hint(padapter);
#ifdef CONFIG_RTW_80211R
...
u8 chk_ap_is_alive(_adapter *padapter, struct sta_info *psta)
{
	...
	RTW_INFO(FUNC_ADPT_FMT" rx:"STA_PKTS_FMT", beacon:%llu"
		...
		if (psta != NULL) {
			...
			if (chk_ap_is_alive(padapter, psta) == _FALSE) {
				...
				if (pmlmepriv->cur_network_scanned) {
					_enter_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
					pmlmepriv->cur_network_scanned->network.Rssi = -99;
					_exit_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
				}
			}
...
void roamed_status_chk(_adapter *padapter, u8 from_timer)
{
	...
	if (pmlmepriv->roam_act_cnt == pmlmepriv->roam_scan_int) {
		if (ap->network.qbss_valid) {
			RTW_INFO("ServingAP:* %s("MAC_FMT", ch%3u) rssi:%d dBm, load:%d%c, sta:%d\n",
				 ap->network.Ssid.Ssid,
				 MAC_ARG(ap->network.MacAddress),
				 ap->network.Configuration.DSConfig,
				 translate_percentage_to_dbm(ap->network.PhyInfo.SignalStrength),
				 ap->network.qbss_ch_util*100/255, '%',
				 ap->network.qbss_sta_cnt);
		} else {
			RTW_INFO("ServingAP:* %s("MAC_FMT", ch%3u) rssi:%d dBm, load:-%c, sta:-\n",
				 ap->network.Ssid.Ssid,
				 MAC_ARG(ap->network.MacAddress),
				 ap->network.Configuration.DSConfig,
				 translate_percentage_to_dbm(ap->network.PhyInfo.SignalStrength), '%');
	...
	roam_cur_rssi_th = roam_rssi_threshold(pmlmepriv);
	roam_cur_rssi_diff = roam_rssi_diff_th(pmlmepriv);

#ifdef CONFIG_RTW_PREFER_5G
	if (pmlmepriv->roam_rssi_base != 0) {
		if (pmlmepriv->roam_rssi_base >= precvpriv->signal_strength)
			val = pmlmepriv->roam_rssi_base - precvpriv->signal_strength;
		else
			val = precvpriv->signal_strength - pmlmepriv->roam_rssi_base;

		if (pmlmepriv->roam_rssi_base > 30)
			pulse_cur_rssi_diff = roam_cur_rssi_diff;
		else
			pulse_cur_rssi_diff = pmlmepriv->roam_idle_rssi_diff_th;

		//if (val >= pmlmepriv->roam_busy_rssi_diff_th) {
		if (val >= pulse_cur_rssi_diff) {
			/* RSSI pulse */
			RTW_INFO("RSSI pulse = %d (B%u)\n",
				padapter->recvpriv.signal_strength, pmlmepriv->roam_rssi_base);

			skip_time_chk = 1;
			skip_rssi_chk = 1;

			/* reset retry to aggresive */
			pmlmepriv->roam_rssi_base = 0;
			pmlmepriv->roam_try_cnt = 0;
		}
	}
	...
	} else if (pmlmepriv->roam_try_cnt > pmlmepriv->roam_scan_round2) {
		wait_ms = pmlmepriv->roam_scan_int2 * 2000;
	} else if (pmlmepriv->roam_try_cnt > pmlmepriv->roam_scan_round1) {
		wait_ms = pmlmepriv->roam_scan_int1 * 2000;
		if (pmlmepriv->roam_rssi_base == 0)
			pmlmepriv->roam_rssi_base = precvpriv->signal_strength;
	} else {
		wait_ms = pmlmepriv->roam_scan_int * 2000;
	}

	pass_ms = rtw_get_passing_time_ms(pmlmepriv->last_roaming);
	remain_ms = (wait_ms <= pass_ms) ? 0 : ((wait_ms - pass_ms)/2000);
	...
	RTW_INFO("signal_strength = %d (%s,D%u,T%u - B%u,r%u,t%u)\n",
		padapter->recvpriv.signal_strength,
		pmlmepriv->LinkDetectInfo.bBusyTraffic?"busy":"idle", roam_cur_rssi_diff,
		roam_cur_rssi_th, pmlmepriv->roam_rssi_base, pmlmepriv->roam_try_cnt,
		!pmlmepriv->need_to_roam ? 0 : skip_time_chk ? 0 : remain_ms);

	if (skip_rssi_chk || (padapter->recvpriv.signal_strength < roam_cur_rssi_th)) {

		pmlmepriv->need_to_roam = _TRUE;

		if (skip_time_chk || remain_ms == 0) {
#ifdef CONFIG_RTW_80211K
			if (rtw_roam_nb_discover(padapter, _FALSE, 0))
				while (!pmlmepriv->nb_info.nb_rpt_valid && wait_times--)
					mdelay(10); /* max wait 100ms for NB report ready */
#endif
			if (rtw_drv_scan_by_self(padapter, reason)) {
				pmlmepriv->roam_try_cnt++;
				pmlmepriv->last_roaming = rtw_get_current_time();
			}
		}
	} else {
...
#ifdef CONFIG_LAYER2_ROAMING
			|| (pmlmepriv->roam_network && pmlmepriv->roam_ack)
#endif
		) {
			recv_ack(padapter, false);
...
u32 rtw_scan_timeout_decision(_adapter *padapter)
{
	...
	if (IS_ACS_ENABLE(padapter) && rtw_is_acs_st_valid(padapter) &&
			!(ss->reason & RTW_AUTO_SCAN_REASON_ROAM))
		...
		u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num, bool no_sparse, int reason)
{
	...
	if (amount_ch == 0) {
		/* 5G */
		for (i = 0; i < rfctl->max_chan_nums; i++) {
			if (rfctl->channel_set[i].band != BAND_ON_5G)
				continue;
			...
			if (rtw_mlme_band_check(padapter, chan) == _TRUE) {
				...
				if ((reason & RTW_AUTO_SCAN_REASON_ROAM) &&
					rtw_chlist_search_ch(out_roam, r, chan) != -1)
					continue;
				if (0)
		...
		/* 2G */
		for (i = 0; i < rfctl->max_chan_nums; i++) {
			if (rfctl->channel_set[i].band != BAND_ON_2_4G)
				break;
	...
	if (!no_sparse
		&& !regsty->wifi_spec
		&& amount_ch > 6 /* assume ch_num > 6 is normal scan */
	) {
		/* scan_sparse */
		amount_ch = rtw_scan_sparse(padapter, out, amount_ch);
	}
...
static void sitesurvey_res_reset(_adapter *adapter, struct sitesurvey_parm *parm)
{
	...
	ss->token = parm->token;
	ss->reason = parm->reason;
	ss->acs = parm->acs;

#ifdef CONFIG_LAYER2_ROAMING
	ss->nb_num = parm->nb_num;
	for (i = 0; i < ss->nb_num; i++)
		_rtw_memcpy(&ss->nb[i], &parm->nb[i], sizeof(struct nb_bssid));
#endif
}
...
void site_survey(_adapter *padapter, u8 survey_channel, RT_SCAN_TYPE ScanType)
{
	...
	if (ss->reason & RTW_AUTO_SCAN_REASON_ROAM) {
		...
		for (i = 0; i < RTW_SSID_SCAN_AMOUNT; i++) {
			found = 0;
			if (ss->ssid[i].SsidLength) {
				if (ss->nb_num) {
					for (j = 0; j < ss->nb_num; j++) {
						if (survey_channel == ss->nb[j].ch) {
							found++;
							if (padapter->registrypriv.wifi_spec)
								issue_probereq(padapter, &ss->ssid[i], NULL);
							else
								issue_probereq_ex(padapter, &ss->ssid[i],
									(const u8 *)&ss->nb[j].bssid,
									0, 0, 0, 0);
						}
					}
				}
#ifndef DBG_UNIT_TEST_ROAM
				if (!found)
#endif
				{
					...
					if (padapter->registrypriv.wifi_spec)
						...
						issue_probereq_ex(padapter, &ss->ssid[i], NULL, 0, 0, 0, 0);
				}
			}
			found_all += found;
		}
...
operation_by_state:
	...
	case SCAN_DISABLE:
		...
		*/
		...
		/* show current serving AP */
		if (ss->reason & RTW_AUTO_SCAN_REASON_ROAM && ap)
			RTW_INFO("ServingAP:* %s("MAC_FMT", ch%3u) rssi:%d dBm, age:%5d\n",
				 ap->network.Ssid.Ssid,
				 MAC_ARG(ap->network.MacAddress),
				 ap->network.Configuration.DSConfig,
				 translate_percentage_to_dbm(ap->network.PhyInfo.SignalStrength),
				 rtw_get_passing_time_ms(ap->last_scanned));
	...
	case SCAN_PROCESS: {
		...
		if ((ss->reason & RTW_AUTO_SCAN_REASON_ROAM) &&
			(next_state == SCAN_PROCESS)) {
			if ((scan_ch < 36) && pmlmepriv->candidate_5G) {
				RTW_INFO("%s() has 2'nd roam candidate, abort scan!\n", __func__);
				next_state = SCAN_COMPLETE;
		...
		if (ss->reason & RTW_AUTO_SCAN_REASON_ROAM && scan_type == SCAN_ACTIVE)
			scan_ms = 20;
		else
			scan_ms = ss->scan_ch_ms;
		...
		#ifdef DBG_SITESURVEY
			#ifdef CONFIG_P2P
			RTW_INFO(FUNC_ADPT_FMT" %s ch:%u (cnt:%u,idx:%d) at %dms, %c%c%c%c, R=%d(%dms)\n"
				, FUNC_ADPT_ARG(padapter)
