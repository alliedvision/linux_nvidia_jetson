...
#define RTW_CHD_2G_MAX 13
#define RTW_CHD_5G_MAX 73
#define RTW_CHPLAN_HEXFILE_MAX 128
...
static struct ch_list_t rtw_channel_def_2g[RTW_CHD_2G_MAX];

#ifdef CONFIG_IEEE80211_BAND_5GHZ
static struct ch_list_t rtw_channel_def_5g[RTW_CHD_5G_MAX];
...
int rtw_get_channel_plan_from_file(const char *path)
{
	...
	while (i < len) {
		if (j < sec_1) {
			rtw_channel_def_2g[j].len_ch_attr = rtw_hex_setting_buf + i;
			if (rtw_hex_setting_buf[i])
				i += (rtw_hex_setting_buf[i] + 2);
			else
				i += (rtw_hex_setting_buf[i] + 1);
		} else if (j >= sec_1 && j < sec_2) {
			idx = j - sec_1;
			rtw_channel_def_5g[idx].len_ch_attr = rtw_hex_setting_buf + i;
			if (rtw_hex_setting_buf[i])
				i += (rtw_hex_setting_buf[i] + 2);
			else
				i += (rtw_hex_setting_buf[i] + 1);
			...
			if (i + 2 >= len)
				return -1;
			idx = j - sec_2;
			RTW_ChannelPlanMap[idx].chd_2g =
						*(rtw_hex_setting_buf + i);
			RTW_ChannelPlanMap[idx].chd_5g =
						*(rtw_hex_setting_buf + i + 1);
			RTW_ChannelPlanMap[idx].regd_2g =
						*(rtw_hex_setting_buf + i + 2);
			RTW_ChannelPlanMap[idx].regd_5g =
...
bool rtw_is_channel_plan_valid(u8 id)
{
	if (id < RTW_ChannelPlanMap_size) {
		...
		/* check that chplan is not empty */
		if (chplan_map->chd_2g != 8
			#if CONFIG_IEEE80211_BAND_5GHZ
			&& chplan_map->chd_5g != 0
			#endif
		)
...
bool rtw_chinfo_allow_beacon_hint(struct _RT_CHANNEL_INFO *chinfo, u8 reason)
{
	return (RTW_CHPLAN_BEACON_HINT_ON_2G_CH_1_11 || !(chinfo->band == BAND_ON_2_4G && chinfo->ChannelNum <= 11))
		&& (RTW_CHPLAN_BEACON_HINT_ON_DFS_CH ||
			!(chinfo->flags & RTW_CHF_DFS) ||
			reason == RTW_AUTO_SCAN_REASON_ROAM);
}

u8 rtw_process_beacon_hint(_adapter *adapter, WLAN_BSSID_EX *bss, u8 reason)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	...
	if ((chinfo->flags & RTW_CHF_NO_IR)
		...
		&& rtw_chinfo_allow_beacon_hint(chinfo, reason)
	) {
...
u8 rtw_clr_dfs_beacon_hint(_adapter *adapter)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	RT_CHANNEL_INFO *chset = rfctl->channel_set;
	int i;
