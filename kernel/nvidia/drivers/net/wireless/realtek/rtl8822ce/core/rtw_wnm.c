...
u8 rtw_wmn_btm_rsp_reason_decision(_adapter *padapter, u8* req_mode)
{
	...
	u8 reason = 0;
	u8 roam_cur_rssi_threshold;

	if (!rtw_wnm_btm_diff_bss(padapter)) {
		/* Reject - No suitable BSS transition candidates */
		RTW_WNM_INFO("tgt_addr:"MAC_FMT"cur_addr:"MAC_FMT"\n",
			MAC_ARG(pmlmepriv->nb_info.roam_target_addr),
			MAC_ARG(pmlmepriv->cur_network.network.MacAddress));
		reason = 7;
		RTW_WNM_INFO("%s : Reject - Different BSS\n", __func__);
	...
	if (((*req_mode) & DISASSOC_IMMINENT) == 0) {
		...
		reason = 1;
		RTW_WNM_INFO("%s : Reject - DISASSOC_IMMINENT\n", __func__);
		goto candidate_remove;
	}

	roam_cur_rssi_threshold = (pmlmepriv->LinkDetectInfo.bBusyTraffic)
		? (pmlmepriv->roam_busy_rssi_threshold)
		: (pmlmepriv->roam_idle_rssi_threshold);
	if (precvpriv->signal_strength_data.avg_val >= roam_cur_rssi_threshold) {
		reason = 1;
		RTW_WNM_INFO("%s : Reject - under high roam rssi(%u, %u) \n",
			__func__, precvpriv->signal_strength_data.avg_val,
			roam_cur_rssi_threshold);
		goto candidate_remove;
	}
...
void rtw_wnm_roam_scan_hdl(void *ctx)
{
	...
	if (rtw_is_scan_deny(padapter)) {
		RTW_WNM_INFO("%s: scan_deny! postpone roam\n", __func__);
		_set_timer(&pnb->roam_scan_timer, 500);
		return;
	}
	...
	if (rtw_drv_scan_by_self(padapter, RTW_AUTO_SCAN_REASON_ROAM)) {
...
void rtw_wnm_process_btm_req(_adapter *padapter, u8* pframe, u32 frame_len)
{
	...
	if (reason == 0) {
		...
		if (wnm_btm_bss_disassoc_im(pframe)) {
			RTW_WNM_INFO("%s : wait AP dissociation\n", __func__);
			_set_timer(&pnb->disassoc_chk_timer, wnm_disassoc_wait_time);
		} else {
			RTW_WNM_INFO("%s : roaming immediately\n", __func__);
			_set_timer(&pnb->disassoc_chk_timer, 1);
		}
	}
...
static void rtw_wnm_nb_info_update(_adapter *adapter,
	...
	u8 *nb_rpt_is_same)
{
	...
	for (i=0; i<nb_rpt_entries; i++) {
		...
		bss.Configuration.DSConfig = pnb->nb_rpt[i].ch_num;
		rtw_process_beacon_hint(adapter, &bss, RTW_AUTO_SCAN_REASON_ROAM);
	}
}
...
static void rtw_wnm_btm_candidate_select(_adapter *padapter)
{
	...
	if (bfound) {
		...
		RTW_INFO("WNM : btm entry isn't in scan queue\n");
		_rtw_memset(pnb->roam_target_addr,0, ETH_ALEN);
	}
}
