...
/* configurations about TX power */
	...
	RTW_PRINT_SEL(sel, "CONFIG_RTW_MBO\n");
#endif
	...
	RTW_PRINT_SEL(sel, "CONFIG_RTW_WNM\n");
#endif
...
int proc_get_roam_param(struct seq_file *m, void *v)
{
	...
	RTW_PRINT_SEL(m, "%16s %18s %13s %21s %13s %13s\n"
		, "idle_rssi_diff_th"
		, "busy_rssi_diff_th"
		, "scanr_exp_ms"
		, "scan_int(unit:2 sec)"
		, "idle_rssi_th"
		, "busy_rssi_th");
	RTW_PRINT_SEL(m, "%-18u %-18u %-13u %-21u %-13u %-13u\n"
		, mlme->roam_idle_rssi_diff_th
		, mlme->roam_busy_rssi_diff_th
		, mlme->roam_scanr_exp_ms
		, mlme->roam_scan_int
		, mlme->roam_idle_rssi_threshold
...
ssize_t proc_set_roam_param(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	char tmp[32];
	u8 idle_rssi_diff_th;
	u8 busy_rssi_diff_th;
	u32 scanr_exp_ms;
	u32 scan_int;
	u8 idle_rssi_threshold;
	u8 busy_rssi_threshold;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (num >= 1)
			mlme->roam_idle_rssi_diff_th = idle_rssi_diff_th;
		if (num >= 2)
			mlme->roam_busy_rssi_diff_th = busy_rssi_diff_th;
		if (num >= 3)
			mlme->roam_scanr_exp_ms = scanr_exp_ms;
		if (num >= 4)
			mlme->roam_scan_int = scan_int;
		if (num >= 5)
			mlme->roam_idle_rssi_threshold = idle_rssi_threshold;
		if (num >= 6)
			mlme->roam_busy_rssi_threshold = busy_rssi_threshold;
	}
...
int proc_get_roam_param_int(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct mlme_priv *mlme = &adapter->mlmepriv;

	RTW_PRINT_SEL(m, "%20s %22s %22s %12s %12s %12s\n"
		, "scan_int1(unit:2 sec)"
		, "scan_int2(unit:2 sec)"
		, "scan_int3(unit:2 sec)"
		, "scan_round1"
		, "scan_round2"
		, "scan_round3");
	RTW_PRINT_SEL(m, "%-22u %-22u %-22u %-12u %-12u %-12u\n"
		, mlme->roam_scan_int1
		, mlme->roam_scan_int2
		, mlme->roam_scan_int3
		, mlme->roam_scan_round1
		, mlme->roam_scan_round2
		, mlme->roam_scan_round3
	);

	return 0;
}

ssize_t proc_set_roam_param_int(struct file *file, const char __user *buffer,
	size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		int num = sscanf(tmp, "%u %u %u %u %u %u"
			, &scan_int1
			, &scan_int2
			, &scan_int3
			, &scan_round1
		...
		if (num >= 1)
			mlme->roam_scan_int1 = scan_int1;
		if (num >= 2)
			mlme->roam_scan_int2 = scan_int2;
		if (num >= 3)
			mlme->roam_scan_int3 = scan_int3;
		if (num >= 4)
			mlme->roam_scan_round1 = scan_round1;
		if (num >= 5)
			mlme->roam_scan_round2 = scan_round2;
		if (num >= 6)
			mlme->roam_scan_round3 = scan_round3;
	}
...
int proc_get_roam_tgt_addr(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	u8 *addr = adapter->mlmepriv.roam_tgt_addr;

	RTW_PRINT_SEL(m, "roam target addr: %02x:%02x:%02x:%02x:%02x:%02x\n",
...
int proc_get_tx_ring_ext(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < HW_QUEUE_ENTRY; i++) {
		...
		for (j = 0; j < TX_BAK_FRMAE_CNT; j++) {
			RTW_PRINT_SEL(m, "  txbd[%03d]:\n", j);
			for (k = 0; k < TX_BAK_TXBD_LEN / 4; k++) {
				if ((k % 4) == 0)
					RTW_PRINT_SEL(m, "  0x%03x", k);

				RTW_PRINT_SEL(m, " 0x%08x ", ((int *)pbuf->tx_bak_txbd)[k]);
			...
			RTW_PRINT_SEL(m, "  R/W pointer: %d/%d, status=0x%08x\n",
				pbuf->tx_bak_rp, pbuf->tx_bak_wp, pbuf->tx_dma_status);
...
int core_get_tx_ring_ext(_adapter *padapter)
{
	_irqL irqL;
	struct dvobj_priv *pdvobjpriv = adapter_to_dvobj(padapter);
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	...
	printk("<<<< tx ring ext dump settings (dump_txbd_desc=%d) >>>>\n", pxmitpriv->dump_txbd_desc);
	printk(" - backup frame num: %d\n", TX_BAK_FRMAE_CNT);
	printk(" - backup max. desc size: %d bytes\n", TX_BAK_DESC_LEN);
	printk(" - backup data size: %d bytes\n\n", TX_BAK_DATA_LEN);

	if (pxmitpriv->dump_txbd_desc == 0) {
		printk("Dump function is disabled.\n");
		return 0;
	}
	...
	for (i = 0; i < HW_QUEUE_ENTRY; i++) {
		...
		sprintf(line + strlen(line), "Tx ring[%d]", i);
		switch (i) {
		case 0:
			sprintf(line + strlen(line), " (VO)\n");
			break;
		case 1:
			sprintf(line + strlen(line), " (VI)\n");
			break;
		case 2:
			sprintf(line + strlen(line), " (BE)\n");
			break;
		case 3:
			sprintf(line + strlen(line), " (BK)\n");
			break;
		case 4:
			sprintf(line + strlen(line), " (BCN)\n");
			break;
		case 5:
			sprintf(line + strlen(line), " (MGT)\n");
			break;
		case 6:
			sprintf(line + strlen(line), " (HIGH)\n");
			break;
		case 7:
			sprintf(line + strlen(line), " (TXCMD)\n");
			break;
		default:
		...
		for (j = 0; j < TX_BAK_FRMAE_CNT; j++) {
			...
			for (k = 0; k < TX_BAK_TXBD_LEN / 4; k++) {
				if ((k % 4) == 0)
					sprintf(line + strlen(line), "  0x%03x", k);

				sprintf(line + strlen(line), " 0x%08x ", ((int *)pbuf->tx_bak_txbd)[k]);
			...
			for (k = 0; k < (pbuf->tx_desc_size) / 4; k++) {
				...
				sprintf(line + strlen(line), " 0x%08x ", ((int *)pbuf->tx_bak_desc)[k]);
			...
			if (pbuf->tx_desc_size) {
				...
				for (k = 0; k < (TX_BAK_DATA_LEN) / 4; k++) {
					...
					sprintf(line + strlen(line), " 0x%08x ", ((int *)pbuf->tx_bak_data_hdr)[k]);
			...
			printk("  R/W pointer: %d/%d , status=0x%08x\n",
				pbuf->tx_bak_rp, pbuf->tx_bak_wp, pbuf->tx_dma_status);
