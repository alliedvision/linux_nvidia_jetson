...
retry:
	...
	while (--i >= 0) {
		...
		if (!err)
			continue;
		pr_err("%s:%d: %p failed err:%d. Could be fatal!!\n",
			 __func__, __LINE__,
...
static int _tegra_set_vpr_params(void *vpr_base, size_t vpr_size)
{
	...
	if (retval != 0) {
		pr_err("%s: smc failed, base %p size %zx, err (0x%x)\n",
...
unlock:
	...
	pr_err("%p,%p failed to register to be called before vpr resize!!\n",
		do_idle, do_unidle);
}
