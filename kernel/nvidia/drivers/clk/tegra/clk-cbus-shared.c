...
#else
	current_div = div_frac_get(div_rate, div_parent_rate,
#endif
				sclk_frac_div->width,
...
#else
	divider = div_frac_get(rate, source_rate, width, frac, flags);
#endif
	if (divider < 0) {
...
static void
#endif
clk_shared_debug(struct clk_hw *hw, struct dentry *dir)
{
	struct tegra_clk_cbus_shared *shared = to_clk_cbus_shared(hw);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	struct dentry *d;
	...
	d =
#endif
	debugfs_create_x32("clk_shared_bus_flags", S_IRUGO, dir,
			&shared->flags);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	if (!d)
		return -EINVAL;
	...
	d =
#endif
	debugfs_create_file("clk_possible_rates", S_IRUGO, dir,
