...
struct tps61280_chip {
	...
	struct mutex			mutex; /* mutex for regulator */
 ...
 */
static bool find_voltage_set_register(struct tps61280_chip *tps, int req_vsel,
				      int *vout_reg, int *gpio_val)
{
	int i;
	bool found = false;
...
static int tps61280_dvs_init(struct tps61280_chip *tps61280)
{
	...
	if (init_uv >= TPS61280_VOUT_VMIN && init_uv <= TPS61280_VOUT_VMAX) {
				  ...
				  struct tps61280_platform_data *pdata)
{
	...
	if (pdata->vsel_controlled_mode || pdata->vsel_controlled_dvs) {
		pdata->vsel_gpio = of_get_named_gpio(np, "ti,vsel-pin-gpio", 0);
	...
	if (pdata->enable_pin_ctrl) {
		pdata->enable_gpio = of_get_named_gpio(np, "ti,en-pin-gpio", 0);
