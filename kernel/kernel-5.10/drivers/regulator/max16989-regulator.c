				      ...
				      struct max16989_chip *mchip)
{
	...
	pdata->enable_sync_output =
			of_property_read_bool(np, "maxim,enable-sync-output");

	pdata->enable_clock_ss =
		of_property_read_bool(np, "maxim,enable-clk-spread-spectrum");
	...
	if (pdata->fpwm_sync_gpio == -EINVAL)
		pdata->enable_external_fpwm =
			of_property_read_bool(np, "maxim,enable-external-fpwm");
	if (gpio_is_valid(pdata->fpwm_sync_gpio))
