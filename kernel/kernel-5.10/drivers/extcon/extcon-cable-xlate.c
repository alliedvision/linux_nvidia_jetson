...
struct extcon_cable_xlate {
	...
	spinlock_t lock; /* spinlock for xlate extcon */
	struct mutex cable_lock; /* mutex for xlate extcon */
	...
	struct platform_device *pdev)
{
	...
	pdata->out_cable_names = devm_kzalloc(&pdev->dev, (pdata->n_out_cable) *
					      sizeof(*pdata->out_cable_names),
					      GFP_KERNEL);
...
cable_new_states:
	...
	for (count = 0;  count < pdata->n_io_new_states; ++count) {
		...
		ret = of_property_read_u32_index(np, "cable-new-states",
						 count * 5 + 1, &pval);
		if (!ret)
			pdata->io_new_states[count].current_in_cable_state =
				pval;
