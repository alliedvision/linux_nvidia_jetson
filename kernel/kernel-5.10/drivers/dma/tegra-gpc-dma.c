...
/*
 * DMA driver for Nvidia's Tegra186, Tegra194 and Tegra234 GPC DMA controller.
 *
...
struct tegra_dma_channel {
	...
	struct list_head pending_dma_desc;
...
static void tegra_dma_sg_req_put(struct tegra_dma_channel *tdc,
		struct tegra_dma_sg_req *sgreq,	bool lock)
{
	unsigned long flags;
		...
		bool prealloc)
{
	...
	sg_req = devm_kzalloc(tdc2dev(tdc), sizeof(struct tegra_dma_sg_req),
						  GFP_ATOMIC);
	if (!sg_req)
		return NULL;
		...
		bool to_terminate)
{
	...
	if (!list_is_last(&sgreq->node, &tdc->pending_sg_req)) {
		...
		sgreq->skipped = false;
		handle_continuous_head_request(tdc, sgreq, to_terminate);
	}
}
	...
	dma_cookie_t cookie, struct dma_tx_state *txstate)
{
	...
	list_for_each_entry(dma_desc, &tdc->pending_dma_desc, node) {
...
static void tegra_dma_free_chan_resources(struct dma_chan *dc)
{
	...
	struct list_head pending_dma_desc_list;
	unsigned long flags;

	INIT_LIST_HEAD(&dma_desc_list);
	INIT_LIST_HEAD(&sg_req_list);
	INIT_LIST_HEAD(&pending_dma_desc_list);
	...
	list_splice_init(&tdc->pending_dma_desc, &pending_dma_desc_list);
	tdc->config_init = false;
	tdc->isr_handler = NULL;
	tdc->slave_id = -1;
	raw_spin_unlock_irqrestore(&tdc->lock, flags);
					...
					int stream_id)
{
	...
	for (i = 0; i < nr_chans; i++) {
		sid_mask_reg = TEGRA_GPCDMA_COMMON_STREAM_IDx_MASK(sid_bucket) +
...
static int tegra_dma_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < cdata->nr_channels; i++) {
		...
		INIT_LIST_HEAD(&tdc->pending_dma_desc);
