...
static void calibration_timer_cb(struct timer_list *t)
{
	struct tegra_dfll *td = from_timer(td, t, calibration_timer);
...
int tegra_dfll_set_external_floor_mv(int external_floor_mv)
{
	...
	if (tegra_dfll_dev->external_floor_output != new_output) {
		...
		if (tegra_dfll_dev->mode == DFLL_CLOSED_LOOP)
			dfll_request_rate(tegra_dfll_dev, dfll_request_get(tegra_dfll_dev));
	}
...
static void dfll_set_default_params(struct tegra_dfll *td)
{
	...
	WARN_ON(val > DFLL_CONFIG_DIV_MASK);
...
{
	unsigned long rate, v, v_opp, v_min_align;
	int ret = -EINVAL;
	int j, selector, lut;
...
DEFINE_DEBUGFS_ATTRIBUTE(lock_fops, lock_get, lock_set, "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(rate_fops, rate_get, rate_set, "%llu\n");
...
static ssize_t registers_write(struct file *file,
...
static int profiles_show(struct seq_file *s, void *data)
{
	...
	if (td->tune_high_target_rate_min == ULONG_MAX) {
		...
		for (i = 0; i < size; i++) {
			...
			seq_printf(s, " ..%3dC%5ldmV%9lukHz%s\n",
				   tv.temp, tegra_dfll_dev->lut_uv[v] / 1000,
				   (r ? : r_default) / 1000,
				   r ? " (calibrated)"  : "");
		}
	...
	r = td->dvco_rate_floors[i];
	seq_printf(s, "  vmin:%5ldmV%9lukHz%s\n", td->lut_uv[0] / 1000,
...
static struct {
	...
	{ "enable", 0644, &enable_fops },
	{ "lock", 0644, &lock_fops },
	{ "force_out_mv", 0644, &fout_mv_fops },
	{ "external_floor_mv", 0644, &external_floor_mv_fops },
	{ "pmu_undershoot_gb", 0644, &undershoot_fops },
	{ "tune_high_mv", 0644, &tune_high_mv_fops },
	{ "calibr_delay", 0644, &calibr_delay_fops },
	{ "rate", 0444, &rate_fops },
	{ "dvco_rate_min", 0444, &dvco_rate_min_fops },
	{ "registers", 0444, &registers_fops },
	{ "vmin_mv", 0444, &vmin_fops },
	{ "vmax_mv", 0444, &vmax_fops },
	{ "output_mv", 0444, &output_fops },
	{ "profiles", 0444, &profiles_fops },
	{ "calibrate_floors", 0200, &calibrate_floors_fops },
};
...
{
	return tegra_dfll_dev->lut_uv[tegra_dfll_dev->thermal_floor_output] / 1000;
}
...
{
	return tegra_dfll_dev->lut_uv[tegra_dfll_dev->thermal_cap_output] / 1000;
}
