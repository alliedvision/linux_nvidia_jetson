...
/*
 * edge select info is passed from DT. This is needed to configure mono amps
 * differently for a true stereo playback
 */
static int tas2552_parse_dt(struct tas2552_data *tas2552,
			    struct device_node *np)
{
	return of_property_read_u32(np, "tas2552,pdm_edge_select",
				    &tas2552->edge_select);
}
...
static int tas2552_component_probe(struct snd_soc_component *component)
{
	...
	if (tas2552->edge_select)
		snd_soc_component_write(component, TAS2552_PDM_CFG,
			TAS2552_PDM_CLK_SEL_IVCLKIN | TAS2552_PDM_DATA_ES);
